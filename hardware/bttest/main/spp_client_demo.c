...
// Update LED colors based on the received notification
void update_led_colors(const char *notification)
{
    uint8_t r = 0, g = 0, b = 0;
    int toggle = 0, brightness = 100;

    // Parse the notification string
    if (sscanf(notification, "#%02hhx%02hhx%02hhx,%d,%d", &r, &g, &b, &toggle, &brightness) == 5)
    {
        ESP_LOGI("LEDC", "Parsed: R=%d, G=%d, B=%d, Toggle=%d, Brightness=%d", r, g, b, toggle, brightness);

        // Scale brightness (0-100% maps to 0-255 for duty cycle)
        uint8_t scaled_brightness = (brightness * 255) / 100;

        if (toggle == 0) // RGB Mode
        {
            // Apply brightness scaling to RGB
            ledc_set_duty(LEDC_MODE, LEDC_CHANNEL_R, (r * scaled_brightness) / 255);
            ledc_update_duty(LEDC_MODE, LEDC_CHANNEL_R);

            ledc_set_duty(LEDC_MODE, LEDC_CHANNEL_G, (g * scaled_brightness) / 255);
            ledc_update_duty(LEDC_MODE, LEDC_CHANNEL_G);

            ledc_set_duty(LEDC_MODE, LEDC_CHANNEL_B, (b * scaled_brightness) / 255);
            ledc_update_duty(LEDC_MODE, LEDC_CHANNEL_B);

            // Turn off white channel
            ledc_set_duty(LEDC_MODE, LEDC_CHANNEL_3, 0);
            ledc_update_duty(LEDC_MODE, LEDC_CHANNEL_3);
        }
        else if (toggle == 1) // White Mode
        {
            ...
            // Set white channel brightness on pin 3
            ledc_set_duty(LEDC_MODE, LEDC_CHANNEL_3, scaled_brightness);
            ledc_update_duty(LEDC_MODE, LEDC_CHANNEL_3);
        }

        // Ensure pin 7 is always off
        ledc_set_duty(LEDC_MODE, LEDC_CHANNEL_5, 0);
        ledc_update_duty(LEDC_MODE, LEDC_CHANNEL_5);
    }
    else
    {
        ESP_LOGE("LEDC", "Invalid notification format: %s", notification);
    }
}
...
static void notify_event_handler(esp_ble_gattc_cb_param_t *p_data)
{
    ...
    {
        ESP_LOGI(GATTC_TAG, "+NOTIFY:handle = %d,length = %d", handle, p_data->notify.value_len);
    }

    if (db == NULL)
    {
        ESP_LOGE(GATTC_TAG, "DB is NULL");
        return;
    }

    if (handle == db[SPP_IDX_SPP_DATA_NTY_VAL].attribute_handle)
    {
        // Ensure null-termination of the received string
        char notification[20] = {0};
        memcpy(notification, p_data->notify.value, p_data->notify.value_len > 19 ? 19 : p_data->notify.value_len);

        // Update LEDs with the parsed notification
        update_led_colors(notification);
    }
}
...
void init_led_pwm()
{
    // Configure the timer for LED PWM
    ledc_timer_config_t ledc_timer = {
        .speed_mode = LEDC_MODE,
        .duty_resolution = LEDC_TIMER_12_BIT, // 8-bit resolution (0-255)
        .timer_num = LEDC_TIMER,
        .freq_hz = 2000, // 5 kHz frequency
        .clk_cfg = LEDC_AUTO_CLK,
    };
    ESP_ERROR_CHECK(ledc_timer_config(&ledc_timer));

    // Configure the channels for LED PWM
    ledc_channel_config_t ledc_channel[] = {
        {.channel = LEDC_CHANNEL_R, .gpio_num = GPIO_R, .speed_mode = LEDC_MODE, .timer_sel = LEDC_TIMER, .duty = 0},
        {.channel = LEDC_CHANNEL_G, .gpio_num = GPIO_G, .speed_mode = LEDC_MODE, .timer_sel = LEDC_TIMER, .duty = 0},
        {.channel = LEDC_CHANNEL_B, .gpio_num = GPIO_B, .speed_mode = LEDC_MODE, .timer_sel = LEDC_TIMER, .duty = 0},
        {.channel = LEDC_CHANNEL_3, .gpio_num = 3, .speed_mode = LEDC_MODE, .timer_sel = LEDC_TIMER, .duty = 0}, // White channel
        {.channel = LEDC_CHANNEL_5, .gpio_num = 7, .speed_mode = LEDC_MODE, .timer_sel = LEDC_TIMER, .duty = 0}, // Pin 7 (always off)
    };
...
void app_main(void)
{
    ...
    // Register BLE client application
