...
static const char **current_color = &color_red;
...
// Send notification with RGB, toggle, and brightness
static void send_color_notification()
{
    if (is_connected && enable_data_ntf)
    {
        ...
        // Determine the color to send
        if (toggle == 0) // RGB mode
        {
            hsv_to_rgb(hue, 1.0, (float)brightness / 100.0, &r, &g, &b);
            snprintf(notification, sizeof(notification), "#%02X%02X%02X,%d,%d", r, g, b, toggle, brightness);
        }
        else // White mode
        {
            r = g = b = (uint8_t)(255 * (float)brightness / 100.0);
            snprintf(notification, sizeof(notification), "#%02X%02X%02X,%d,%d", r, g, b, toggle, brightness);
        }
        ...
        ESP_LOGI(TAG, "Sent: %s", notification);
    }
}


// Task to toggle and notify color codes
void toggle_color_task(void *arg)
{
    while (1)
    {
        // Alternate colors
...
// Function to send rainbow color notifications
static void send_rainbow_notification()
{
    if (is_connected && enable_data_ntf)
    {
        uint8_t r, g, b;
        hsv_to_rgb(hue, 1.0, 1.0, &r, &g, &b); // Convert hue to RGB

        char color_code[8];
        snprintf(color_code, sizeof(color_code), "#%02X%02X%02X", r, g, b);
        ...
        ESP_LOGI(TAG, "Sent Color: %s", color_code);

        // Increment hue for the next color (wrap around at 360 degrees)
        hue += 1.0;
        if (hue >= 360.0)
        {
            hue = 0.0;
...
// // Task to handle rainbow cycling
// void rainbow_cycle_task(void *arg)
// {
//     while (1)
//     {
//         send_rainbow_notification();
//         vTaskDelay(pdMS_TO_TICKS(100)); // Smooth transition (adjust delay for speed)
//     }
// }

// Task to cycle colors, brightness, and toggle
void rainbow_brightness_task(void *arg)
{
    while (1)
    {
        for (int i = 0; i < 360; i++) // Rainbow phase
        {
            send_color_notification();
            ...
            // Update brightness
            brightness += brightness_direction * 5; // Change brightness in steps of 5
            if (brightness <= 0 || brightness >= 100)
            {
                brightness_direction *= -1; // Reverse direction
            }

            vTaskDelay(pdMS_TO_TICKS(100)); // Adjust for smooth transitions
        }

        // Switch toggle after rainbow cycle
        toggle = (toggle == 0) ? 1 : 0;

        for (int i = 0; i < 20; i++) // White phase
        {
            send_color_notification();

            // Update brightness
            brightness += brightness_direction * 5;
            if (brightness <= 0 || brightness >= 100)
            {
                brightness_direction *= -1;
            }

            vTaskDelay(pdMS_TO_TICKS(200)); // Slightly slower during white phase
        }

        // Switch toggle back to rainbow
        toggle = (toggle == 0) ? 1 : 0;
    }
}
...
static void spp_task_init(void)
{
    ...
    // Add the rainbow brightness task
    xTaskCreate(rainbow_brightness_task, "rainbow_brightness_task", 4096, NULL, 10, NULL);
}
